...
void CreateRandomMon(struct Pokemon *mon, enum Species species, u8 level, bool32 sanitizeAbility)
{
    CreateRandomMonWithIVs(mon, species, level, USE_RANDOM_IVS, sanitizeAbility);
}

void CreateRandomMonWithIVs(struct Pokemon *mon, enum Species species, u8 level, u8 fixedIv, bool32 sanitizeAbility)
{
    CreateMonWithIVs(mon, species, level, Random32(), OTID_STRUCT_PLAYER_ID, fixedIv, sanitizeAbility);
    GiveMonInitialMoveset(mon);
}

void CreateMon(struct Pokemon *mon, enum Species species, u8 level, u32 personality, struct OriginalTrainerId trainerId, bool32 sanitizeAbility)
{
    u32 mail;
    ZeroMonData(mon);
...
void CreateMonWithIVs(struct Pokemon *mon, enum Species species, u8 level, u32 personality, struct OriginalTrainerId trainerId, u8 fixedIV, bool32 sanitizeAbility)
{
...
void CreateBoxMon(struct BoxPokemon *boxMon, enum Species species, u8 level, u32 personality, struct OriginalTrainerId trainerId, bool32 sanitizeAbility)
{
...
void CreateMaleMon(struct Pokemon *mon, enum Species species, u8 level, bool32 sanitizeAbility)
{
...
void CreateMonWithIVsPersonality(struct Pokemon *mon, enum Species species, u8 level, u32 ivs, u32 personality, bool32 sanitizeAbility)
{
...
static void CreateEventMon(struct Pokemon *mon, enum Species species, u8 level, u32 personality, struct OriginalTrainerId otId)
{
    ...
    CreateMonWithIVs(mon, species, level, personality, otId, USE_RANDOM_IVS, FALSE);
    GiveMonInitialMoveset(mon);
    ...
    if (species && species != SPECIES_EGG)
    {
        ...
        if (event == FRIENDSHIP_EVENT_LEAGUE_BATTLE)
        {
            ...
            if (!(opponentTrainerClass == TRAINER_CLASS_LEADER
                || opponentTrainerClass == TRAINER_CLASS_KANTONIAN
...
void TryTyrogueEvo(void)
{
    ...
    gTriedEvolving |= 1u << gSpecialVar_0x8004;
    if(gMain.callback2 == TrySpecialOverworldEvo) // This fixes small graphics glitches.
        EvolutionScene(&gParties[B_TRAINER_PLAYER][gSpecialVar_0x8004], targetSpecies, canStopEvo, gSpecialVar_0x8004);
    else
        BeginEvolutionScene(&gParties[B_TRAINER_PLAYER][gSpecialVar_0x8004], targetSpecies, canStopEvo, gSpecialVar_0x8004);
