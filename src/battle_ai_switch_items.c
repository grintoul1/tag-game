...
extern u16 GetAbilityBySpecies(u16 species, u8 abilityNum);
...
static u32 CustomGetBestMonIntegrated(struct Pokemon *party, int firstId, int lastId, u32 battler, u32 opposingBattler, u32 battlerIn1, u32 battlerIn2, enum SwitchType switchType)
{
    ...
    {
        u32 maxDamageDealtToAI=0, damageReceived=0, percentageReceived=0, damageDealt=0, percentageDealt=0, maxDamageDealt=0; 
        u32 aiMonSpecies=0;
        u32 aiMonFaster=0;
        u32 aiMove=0;
        u32 opponentMove=0;
        ...
        {
            damageDealt=0;
            aiMove = GetMonData(&party[monId], MON_DATA_MOVE1 + i,NULL);
            if (aiMove != MOVE_NONE)
            {
                // Only check damage if it's a damaging move
                if (!IsBattleMoveStatus(aiMove))
