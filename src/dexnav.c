...
//////////////////////////////
static void CreateDexNavWildMon(enum Species species, u8 potential, u8 level, u8 abilityNum, enum Item item, enum Move *moves) // grintoul IVs
{
    struct Pokemon *mon = &gParties[B_TRAINER_OPPONENT_A][0];

    CreateWildMon(species, level, TRUE);  // shiny rate bonus handled in CreateBoxMon
...
static void DexNavGenerateMoveset(enum Species species, u8 searchLevel, u8 encounterLevel, u16 *moveDst)
{
    ...
    CreateWildMon(species, encounterLevel, ShouldSanitizeEncounterAbility(species));
