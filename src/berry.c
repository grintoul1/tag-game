...
static u8 CalcBerryYield(struct BerryTree *tree)
{
    ...
    if (tree->berry == BERRY_ID_ORAN
    || tree->berry == BERRY_ID_PECHA
    || tree->berry == BERRY_ID_CHERI
    || tree->berry == BERRY_ID_RAWST
    || tree->berry == BERRY_ID_ASPEAR
    || tree->berry == BERRY_ID_PERSIM
    || tree->berry == BERRY_ID_CHESTO)
        return 99;
    if (tree->berry == BERRY_ID_SITRUS
    || tree->berry == BERRY_ID_LUM)
        return 30;
    if (tree->berry == BERRY_ID_IAPAPA
    || tree->berry == BERRY_ID_WIKI
    || tree->berry == BERRY_ID_MAGO
    || tree->berry == BERRY_ID_FIGY
    || tree->berry == BERRY_ID_AGUAV)
        return 6;
    if (tree->berry == BERRY_ID_CUSTAP)
        return 2;
