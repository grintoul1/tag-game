...
static void CreateHatchedMon(struct Pokemon *egg, struct Pokemon *temp)
{
    ...
    CreateMonWithIVs(temp, species, EGG_HATCH_LEVEL, personality, OTID_STRUCT_PLAYER_ID, USE_RANDOM_IVS, TRUE);
    for (i = 0; i < MAX_MON_MOVES; i++)
...
static void CB2_LoadEggHatch(void)
{
    ...
    case 3:
    {
        u32 species = GetMonData(&gParties[B_TRAINER_0][sEggHatchData->eggPartyId], MON_DATA_SPECIES);
