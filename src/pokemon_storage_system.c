...
#define OPTION_MOVE_ELITE_FOUR 8
...
static void Task_PokeStorageMain(u8 taskId)
{
    ...
    case MSTATE_HANDLE_INPUT:
        ...
        case INPUT_SHOW_PARTY:
            if (sStorage->boxOption != OPTION_MOVE_MONS && sStorage->boxOption != OPTION_MOVE_ELITE_FOUR && sStorage->boxOption != OPTION_MOVE_ITEMS && sStorage->boxOption != OPTION_SELECT_MON)
            {
    ...
    if (sStorage->boxOption == OPTION_MOVE_ELITE_FOUR)
    {
        ...
        case 4:
            if (!IsComputerScreenCloseEffectActive())
            {
                ...
                for (u8 i = 0; i < gPartiesCount[B_TRAINER_0]; i++)
                {
                    CopyMon(&gEliteFourPool[i], &gParties[B_TRAINER_0][i], sizeof(*&gParties[B_TRAINER_0][i]));
                }
...
static void Task_ChangeScreen(u8 taskId)
{
    ...
    case SCREEN_CHANGE_EXIT_ELITE_FOUR:
        ...
        {
            gEliteFourPool[i] = gParties[B_TRAINER_0][i];
        }
        FreePokeStorageData();
        DestroyTask(taskId);
...
{
    u16 i, count;
    u16 species = GetMonData(&gParties[B_TRAINER_0][0], MON_DATA_SPECIES);
    ...
    else
    {
        ...
        for (i = 0; i < PARTY_SIZE; i++)
        {
            ...
            {
                personality = GetMonData(&gParties[B_TRAINER_0][i], MON_DATA_PERSONALITY);
                sStorage->partySprites[i] = CreateMonIconSprite(species, personality, 152,  8 * (3 * (i)) + 16, 1, 12, isEgg);
                count++;
            }
            else
            {
