...
bool32 NoAliveMonsForPlayer(void)
{
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_MULTI)
    {
        if (GetConfig(B_MULTI_BATTLE_WHITEOUT) > GEN_3)
            maxIneligible += gPartiesCount[B_TRAINER_2];
    ...
    if (GetConfig(B_MULTI_BATTLE_WHITEOUT) > GEN_3 && gBattleTypeFlags & (BATTLE_TYPE_MULTI | BATTLE_TYPE_INGAME_PARTNER))
    {
        ...
        for (u32 i = 0; i < PARTY_SIZE; i++)
        {
            struct Pokemon mon = gParties[B_TRAINER_2][i];
...
void BS_TryAllySwitch(void)
{
    ...
    if (!IsBattlerAlive(BATTLE_PARTNER(gBattlerAttacker))
     /*|| (GetBattlerSide(gBattlerAttacker) == B_SIDE_OPPONENT && gBattleTypeFlags & BATTLE_TYPE_TWO_OPPONENTS && !(TRAINER_BATTLE_PARAM.opponentB == 0xFFFF))
    ...
    {
        if (GetConfig(B_ALLY_SWITCH_FAIL_CHANCE) >= GEN_9)
        {
