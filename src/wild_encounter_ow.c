...
void StartWildBattleWithOWE(struct ScriptContext *ctx)
{
    ...
    CreateMonWithIVs(&gParties[B_TRAINER_OPPONENT_A][0], speciesId, level, personality, OTID_STRUCT_PLAYER_ID, USE_RANDOM_IVS, ShouldSanitizeEncounterAbility(speciesId));
...
static bool32 CreateEnemyPartyOWE(struct InfoOWE *info, s32 x, s32 y)
{
    ...
    */
    ...
    if (info->category == OWE_CATEGORY_UNDEFINED)
    {
        ...
        {
            CreateWildMon(gWildFeebas.species, ChooseWildMonLevel(&gWildFeebas, 0, WILD_AREA_FISHING), FALSE);
