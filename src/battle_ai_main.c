...
static s32 AI_PartnerTrainer(u32 battlerAtk, u32 battlerDef, enum Move move, s32 score);
static s32 AI_TagOpponent(u32 battlerAtk, u32 battlerDef, enum Move move, s32 score);
static s32 AI_SmartTargeting(u32 battlerAtk, u32 battlerDef, enum Move move, s32 score);
        ...
        case EFFECT_PROTECT:
            {
                 ...
                 && protectMethod != PROTECT_CRAFTY_SHIELD) //These moves have infinite usage
                {
                    ...
                    else if (gBattleMons[battlerAtk].volatiles.protectUses >= 1)
                    {
...
static enum MoveComparisonResult CompareMoveAccuracies(u32 battlerAtk, u32 battlerDef, enum Move moveSlot1, enum Move moveSlot2)
{
...
static enum MoveComparisonResult CompareResistBerryEffects(u32 battlerAtk, u32 battlerDef, enum Move moveSlot1, enum Move moveSlot2)
{
...
static inline bool32 ShouldUseSpreadDamageMove(u32 battlerAtk, enum Move move, enum Move moveIndex, u32 hitsToFaintOpposingBattler)
{
...
static bool32 ShouldCompareMove(u32 battlerAtk, u32 battlerDef, enum Move moveIndex, enum Move move)
{
    if (IsTargetingPartner(battlerAtk, battlerDef))
        return FALSE;
...
static s32 AI_PartnerTrainer(u32 battlerAtk, u32 battlerDef, enum Move move, s32 score)
{
    ...
    bool32 partnerProtecting = (((HasMove(battlerAtkPartner, MOVE_PROTECT)
                        ...
                        || HasMove(battlerAtkPartner, MOVE_ENDURE)) && !gBattleMons[battlerAtk].volatiles.protectUses) && !MoveIgnoresProtect(move));
    bool32 partnerHasBadAbility = (gAbilitiesInfo[atkPartnerAbility].aiRating < 0);
    u32 friendlyFireThreshold = GetFriendlyFireKOThreshold(battlerAtk);
        ...
        // stat raising effects
            ...
            case EFFECT_ATTACK_UP_2: // PARTNER DIFFERENCE - Partner done
                if (!BattlerStatCanRise(battlerAtk, aiData->abilities[battlerAtk], STAT_ATK) || !HasMoveWithCategory(battlerAtk, DAMAGE_CATEGORY_PHYSICAL))
                    ADJUST_AND_RETURN_SCORE(NO_DAMAGE_OR_FAILS);
                break;
            case EFFECT_ATTACK_UP_USER_ALLY: // PARTNER DIFFERENCE - Partner done
                ADJUST_AND_RETURN_SCORE(NO_DAMAGE_OR_FAILS);
                break;
            case EFFECT_STUFF_CHEEKS:
                if (GetItemPocket(gBattleMons[battlerAtk].item) != POCKET_BERRIES)
    ...
    // AI_CalcMoveEffectScore
    {
        ...
        case EFFECT_REST:
            ...
            break;
        case EFFECT_OHKO:
        case EFFECT_SHEER_COLD: // PARTNER DIFFERENCE - Partner done
            if (GetActiveGimmick(battlerDef) == GIMMICK_DYNAMAX)
                break;
        ...
        case EFFECT_ENCORE: // PARTNER DIFFERENCE - Opponent done
        {
            ...
            switch (GetMoveEffect(gLastMoves[battlerDef]))
            {
            case EFFECT_ATTACK_UP:
            case EFFECT_ATTACK_UP_USER_ALLY:
            case EFFECT_DEFENSE_UP:
            ...
            if (gBattleMons[battlerDef].volatiles.encoreTimer == 0
            && (B_MENTAL_HERB < GEN_5 || aiData->holdEffects[battlerDef] != HOLD_EFFECT_MENTAL_HERB)
            && (encourage) && (AI_IsFaster(battlerAtk, battlerDef, move, predictedMoveSpeedCheck, CONSIDER_PRIORITY)))
...
static s32 AI_TagOpponent(u32 battlerAtk, u32 battlerDef, enum Move move, s32 score)
{
    ...
    // AI_CalcMoveEffectScore
    {
        ...
        case EFFECT_MIRROR_MOVE: // PARTNER DIFFERENCE - grintoul TODO
            break;
        case EFFECT_ATTACK_UP_USER_ALLY: // PARTNER DIFFERENCE - Opponent done
            ADJUST_SCORE(IncreaseStatUpScore(BATTLE_PARTNER(battlerAtk), battlerDef, STAT_CHANGE_ATK));
        case EFFECT_ATTACK_UP:
            ADJUST_SCORE(IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_ATK));
            break;
        ...
        case EFFECT_REST:
            ...
            break;
        case EFFECT_OHKO:
        case EFFECT_SHEER_COLD: // PARTNER DIFFERENCE - Opponent Done
            if (GetActiveGimmick(battlerDef) == GIMMICK_DYNAMAX)
                break;
            else if (gBattleMons[battlerAtk].volatiles.lockOn)
                ADJUST_SCORE(BEST_EFFECT);
