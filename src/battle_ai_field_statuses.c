...
static enum FieldEffectOutcome BenefitsFromTrickRoom(enum BattlerId battler)
{
    ...
    u32 battlerSideSpeed = gAiLogicData->speedStats[battler] + gAiLogicData->speedStats[GetPartnerBattler(battler)];
    u32 opposingSideSpeed = gAiLogicData->speedStats[GetBattlerLeftFoe(battler)] + gAiLogicData->speedStats[GetBattlerRightFoe(battler)];
    bool32 slowerThanOpponents = (battlerSideSpeed < opposingSideSpeed);
    bool32 trickRoomUp = (gFieldStatuses & STATUS_FIELD_TRICK_ROOM);
    // If either mon is slower, we want Trick Room.
    if ((!trickRoomUp && (slowerThanOpponents))
     || (trickRoomUp && (!slowerThanOpponents))
     || (trickRoomUp && (slowerThanOpponents) && HasMoveWithEffect(GetPartnerBattler(battler), EFFECT_TRICK_ROOM)))
        return FIELD_EFFECT_POSITIVE;
