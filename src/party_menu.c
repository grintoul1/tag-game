...
static void Task_ValidateChosenStoryParty(u8);
...
static void InitPartyMenu(enum PartyMenuType menuType, enum PartyMenuLayout layout, u8 partyAction, bool8 keepCursorPos, u8 messageId, TaskFunc task, MainCallback callback)
{
    ...
    else
    {
        ...
        if ((menuType == PARTY_MENU_TYPE_CHOOSE_HALF)
         || (menuType == PARTY_MENU_TYPE_CHOOSE_HALF_ELITE_FOUR)
         || (menuType == PARTY_MENU_TYPE_CHOOSE_STORY))
        {
    ...
    else if (gPartiesCount[B_TRAINER_PLAYER] != 0)
    {
        ...
        else
        {
            ...
            else if (gPartyMenu.menuType == PARTY_MENU_TYPE_CONTEST)
            {
             ...
             || (gPartyMenu.menuType == PARTY_MENU_TYPE_CHOOSE_HALF_ELITE_FOUR)
             || (gPartyMenu.menuType == PARTY_MENU_TYPE_CHOOSE_STORY))
            {
...
static void DisplayPartyPokemonDataForChooseHalf(u8 slot)
{
    ...
    if (gSpecialVar_0x800A == B_TRAINER_PARTNER)
        party = gEliteFourPool;
    mon = &party[slot];
...
static bool8 PartyBoxPal_PartnerOrDisqualifiedInArena(u8 slot)
{
    if (gPartyMenu.layout == PARTY_LAYOUT_MULTI_FULL_PARTNER
     ...
     || gSpecialVar_0x800A == B_TRAINER_PARTNER)
    {
        return TRUE;
    }
...
void Task_HandleChooseMonInput(u8 taskId)
{
    if (!gPaletteFade.active && MenuHelpers_ShouldWaitForLinkRecv() != TRUE)
    {
        ...
        case B_BUTTON: // Selected Cancel / pressed B
            if (gPartyMenu.menuType != PARTY_MENU_TYPE_CHOOSE_STORY)
...
static void UpdatePartySelectionSingleLayout(s8 *slotPtr, s8 movementDir)
{
    ...
    if (gSpecialVar_0x800A == B_TRAINER_PARTNER)
        count = CalculateEliteFourPoolCount();
    else if (gPartyMenu.layout  == PARTY_LAYOUT_MULTI_FULL_PARTNER)
        count = gPartiesCount[B_TRAINER_PARTNER];
...
static void UpdatePartySelectionOverworldLayout(s8 *slotPtr, s8 movementDir)
{
    ...
    case MENU_DIR_UP:
        ...
        {
            *slotPtr = (gPartiesCount[B_TRAINER_PLAYER] - 1 == 3) ? 2 : gPartiesCount[B_TRAINER_PLAYER] - 1;
            break;
        }
        else if (*slotPtr == 1)
        {
        ...
        else if (*slotPtr == PARTY_SIZE + 1)
        {
            ...
            if (gPartiesCount[B_TRAINER_PLAYER] - 1 == 3)
            {
                *slotPtr = 2;
                break;
            }
    ...
    case MENU_DIR_DOWN:
        ...
        {
            *slotPtr = (gPartiesCount[B_TRAINER_PLAYER] - 1 == 3) ? PARTY_SIZE + 1 : 4;
            break;
        }
        else if (*slotPtr == 3)
        {
    ...
    case MENU_DIR_RIGHT:
        if (*slotPtr == 0)
        {
            ...
            {
                if (GetMonData(&gParties[B_TRAINER_PLAYER][2], MON_DATA_SPECIES) != SPECIES_NONE)
                    *slotPtr = 2;
            }
...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    {
        // Prevent skipping Shelly cutscene
        if (VarGet(VAR_FORTREE_CITY_STATE) == 4)
            break;
...
static u8 GetPartyMenuActionsType(struct Pokemon *mon)
{
    ...
    case PARTY_MENU_TYPE_CHOOSE_STORY:
...
static void CB2_ShowPokemonSummaryScreen(void)
{
    ...
    else if (gPartyMenu.menuType == PARTY_MENU_TYPE_CHOOSE_HALF || gPartyMenu.menuType == PARTY_MENU_TYPE_CHOOSE_HALF_ELITE_FOUR || gPartyMenu.menuType == PARTY_MENU_TYPE_CHOOSE_STORY)
    {
        if (gSpecialVar_0x800A == B_TRAINER_PARTNER)
            ShowPokemonSummaryScreen(SUMMARY_MODE_LOCK_MOVES, gEliteFourPool, gPartyMenu.slotId, gEliteFourPoolCount - 1, CB2_ReturnToPartyMenuFromSummaryScreen);
        else
...
static void CursorCb_Status(u8 taskId)
{
    ...
    PartyMenuRemoveWindow(&sPartyMenuInternal->windowId[0]);
    PartyMenuRemoveWindow(&sPartyMenuInternal->windowId[1]);
    SetPartyMonSelectionActions(gParties[B_TRAINER_PLAYER], gPartyMenu.slotId, ACTIONS_STATUS);
...
{
    s16 *data = gTasks[taskId].data;
    s16 maxHP=gParties[B_TRAINER_PLAYER][gPartyMenu.slotId].maxHP;
    struct Pokemon *mon=&gParties[B_TRAINER_PLAYER][gPartyMenu.slotId];
...
static void CursorCb_Damage(u8 taskId)
{
    ...
    PartyMenuRemoveWindow(&sPartyMenuInternal->windowId[0]);
    PartyMenuRemoveWindow(&sPartyMenuInternal->windowId[1]);
    SetPartyMonSelectionActions(gParties[B_TRAINER_PLAYER], gPartyMenu.slotId, ACTIONS_DAMAGE);
...
static void CursorCb_Poison(u8 taskId)
{
    ...
    PartyMenuRemoveWindow(&sPartyMenuInternal->windowId[0]);
    PartyMenuRemoveWindow(&sPartyMenuInternal->windowId[1]);
    SetPartyMonSelectionActions(gParties[B_TRAINER_PLAYER], gPartyMenu.slotId, ACTIONS_POISON);
    ...
    else
    {
        ...
        if (sFinalLevel > sInitialLevel)
        {
            ...
            else // Super Candy
            {
                ConvertIntToDecimalStringN(gStringVar2, sExpCandyExperienceTable[holdEffectParam - 1], STR_CONV_MODE_LEFT_ALIGN, 6);
                ConvertIntToDecimalStringN(gStringVar3, sFinalLevel, STR_CONV_MODE_LEFT_ALIGN, 3);
                StringExpandPlaceholders(gStringVar4, gText_PkmnWasElevatedToLvVar3);
            }
...
void InitChoosePartyForStoryBattle(u8 unused)
{
    ClearSelectedPartyOrder();
    InitPartyMenu(PARTY_MENU_TYPE_CHOOSE_STORY, PARTY_LAYOUT_SINGLE, PARTY_ACTION_CHOOSE_MON, FALSE, PARTY_MSG_CHOOSE_MON, Task_HandleChooseMonInput, gMain.savedCallback);
    gPartyMenu.task = Task_ValidateChosenStoryParty;
}
...
static u8 GetPartySlotEntryStatus(s8 slot)
{
    ...
    if (gSpecialVar_0x800A == B_TRAINER_PARTNER)
        party = gEliteFourPool;
    if (GetBattleEntryEligibility(&party[slot]) == FALSE)
        return 2;
...
static void Task_ValidateChosenStoryParty(u8 taskId)
{
    u8 msgId = CheckBattleEntriesAndGetMessage();
    ...
    {
        PlaySE(SE_SELECT);
        if (gSpecialVar_0x800A == B_TRAINER_PLAYER)
        {
...
static bool8 TrySwitchInPokemon(void)
{
    ...
    if (!isSharedTeams && IsMultiBattle() == TRUE && !AreMultiPartiesFullTeams())
    {
...
void ChoosePartyMonUtility1(void)
{
    LockPlayerFieldControls();
    FadeScreen(FADE_TO_BLACK, 0);
    CreateTask(Task_ChoosePartyMon, 10);
...
void ChoosePartyMonUtility2(void)
{
    CreateTask(Task_ChoosePartyMon, 10);
}
