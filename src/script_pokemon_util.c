...
void CreateScriptedWildMon(enum Species species, u8 level, enum Item item)
{
    ...
    u32 personality = GetMonPersonality(species,
        ...
        RANDOM_UNOWN_LETTER);
    CreateMonWithIVs(&gParties[B_TRAINER_OPPONENT_A][0], species, level, personality, OTID_STRUCT_PLAYER_ID, USE_RANDOM_IVS, FALSE);
...
void CreateScriptedDoubleWildMon(enum Species species1, u8 level1, enum Item item1, enum Species species2, u8 level2, enum Item item2)
{
    ...
    u32 personality = GetMonPersonality(species1,
        ...
        RANDOM_UNOWN_LETTER);
    CreateMonWithIVs(&gParties[B_TRAINER_OPPONENT_A][0], species1, level1, personality, OTID_STRUCT_PLAYER_ID, USE_RANDOM_IVS, FALSE);
    GiveMonInitialMoveset(&gParties[B_TRAINER_OPPONENT_A][0]);
    if (item1)
    {
        heldItem1[0] = item1;
    ...
    personality = GetMonPersonality(species2,
        ...
        RANDOM_UNOWN_LETTER);
    CreateMonWithIVs(&gParties[B_TRAINER_OPPONENT_A][1], species2, level2, personality, OTID_STRUCT_PLAYER_ID, USE_RANDOM_IVS, FALSE);
...
static void CB2_ReturnFromChooseHalfPartyEliteFour(void)
{
    ...
    default:
        for (u32 i = 0; i < 3; i++)
        {
            ...
            {
                CopyMon(&gParties[B_TRAINER_OPPONENT_A][gSelectedOrderFromParty[i] - 1], &gParties[B_TRAINER_PLAYER][gSelectedOrderFromParty[i] - 1], sizeof(*&gParties[B_TRAINER_PLAYER][gSelectedOrderFromParty[i] - 1]));
                ZeroMonData(&gParties[B_TRAINER_PLAYER][gSelectedOrderFromParty[i] - 1]);
