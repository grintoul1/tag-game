...
void CreateScriptedDoubleWildMon(enum Species species1, u8 level1, enum Item item1, enum Species species2, u8 level2, enum Item item2)
{
    ...
    personality = GetMonPersonality(species2,
        ...
        RANDOM_UNOWN_LETTER);
    CreateMonWithIVs(&gParties[B_TRAINER_1][1], species2, level2, personality, OTID_STRUCT_PLAYER_ID, USE_RANDOM_IVS, FALSE);
...
static void CB2_ReturnFromChooseHalfPartyEliteFour(void)
{
    ...
    default:
        for (u32 i = 0; i < 3; i++)
        {
            ...
            {
                CopyMon(&gParties[B_TRAINER_1][gSelectedOrderFromParty[i] - 1], &gParties[B_TRAINER_0][gSelectedOrderFromParty[i] - 1], sizeof(*&gParties[B_TRAINER_0][gSelectedOrderFromParty[i] - 1]));
                ZeroMonData(&gParties[B_TRAINER_0][gSelectedOrderFromParty[i] - 1]);
