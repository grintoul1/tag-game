      ...
      /* || HasPartnerTrainer(cv->battlerAtk)*/
        ...
        else if (GetConfig(B_ALLY_SWITCH_FAIL_CHANCE) >= GEN_9)
        {
            ...
            else
            {
                ...
                if (!(GetBattlerSide(cv->battlerAtk) == B_SIDE_PLAYER) 
                || ((GetBattlerSide(cv->battlerAtk) == B_SIDE_PLAYER) && !isSharedTeams))
                {
                    // Switch Battle Controllers
                    SwapBattlerBattleControllers(cv->battlerAtk, GetPartnerBattler(cv->battlerAtk));
                    
                    // Switch AI flags
                    SwapBattlerAiFlags(cv->battlerAtk, GetPartnerBattler(cv->battlerAtk));
                }
            }
        }
        break;
    ...
    while (gBattleStruct->eventState.moveEndBattler < gBattlersCount)
    {
        ...
        while (gBattleStruct->eventState.moveEndBlock < FAINT_BLOCK_COUNT)
        {
            ...
            case FAINT_BLOCK_END_INVERSION_POLICY:
                if (gBattleMons[battlerDef].volatiles.inversionPolicy)
                {
                    gBattleMons[battlerDef].volatiles.inversionPolicy = FALSE;
