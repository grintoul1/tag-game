...
AI_MULTI_BATTLE_TEST("TAG TEST: MULTI: AI: OPPONENT: SCORING: AI always chooses highest damaging move")
{
    GIVEN {
        AI_FLAGS(AI_FLAG_TAG_TRAINER);
        PLAYER(SPECIES_KINGDRA);
        PLAYER(SPECIES_DIANCIE);
        PLAYER(SPECIES_ORTHWORM);
        PARTNER(SPECIES_KINGDRA);
        PARTNER(SPECIES_DIANCIE);
        PARTNER(SPECIES_ORTHWORM);
        OPPONENT_A(SPECIES_WOBBUFFET) { Moves(MOVE_DRAGON_ENERGY, MOVE_MUDDY_WATER, MOVE_HEAT_WAVE); }
        OPPONENT_B(SPECIES_WOBBUFFET) { Moves(MOVE_DRAGON_ENERGY, MOVE_MUDDY_WATER, MOVE_HEAT_WAVE); }
    } WHEN {
...
AI_MULTI_BATTLE_TEST("TAG TEST: MULTI: AI: OPPONENT: SCORING: Fast KO scores +18 80% of the time for opponents")
{
    ...
    GIVEN {
        ...
        PARTNER(SPECIES_KINGDRA){ HP(1); Speed(1); };
        OPPONENT_A(SPECIES_WOBBUFFET) { Moves(MOVE_DRAGON_PULSE, MOVE_SCALD, MOVE_FLAMETHROWER); Speed(2); }
        OPPONENT_B(SPECIES_WOBBUFFET) { Moves(MOVE_DRAGON_PULSE, MOVE_SCALD, MOVE_FLAMETHROWER); Speed(3); }
    } WHEN {
...
AI_TWO_VS_ONE_BATTLE_TEST("TAG TEST: 2VS1: AI: OPPONENT: SCORING: Fast KO scores +18 80% of the time for opponents")
{
    ...
    GIVEN {
        ...
        PARTNER(SPECIES_KINGDRA){ HP(1); Speed(1); };
        OPPONENT_A(SPECIES_WOBBUFFET) { Moves(MOVE_DRAGON_PULSE, MOVE_SCALD, MOVE_FLAMETHROWER); Speed(2); }
        OPPONENT_A(SPECIES_WOBBUFFET) { Moves(MOVE_DRAGON_PULSE, MOVE_SCALD, MOVE_FLAMETHROWER); Speed(3); }
    } WHEN {
            TURN { SCORE_EQ_VAL(opponentLeft, MOVE_DRAGON_PULSE, 118, target:playerLeft); 
                SCORE_EQ_VAL(opponentRight, MOVE_DRAGON_PULSE, 118, target:playerLeft);
                SCORE_EQ_VAL(opponentLeft, MOVE_DRAGON_PULSE, 118, target:playerRight); 
...
AI_MULTI_BATTLE_TEST("TAG TEST: MULTI: AI: OPPONENT: SCORING: Slow KO scores +12 80% of the time for opponents")
{
    ...
    GIVEN {
        ...
        PARTNER(SPECIES_KINGDRA){ HP(1); Speed(3); };
        OPPONENT_A(SPECIES_WOBBUFFET) { Moves(MOVE_DRAGON_PULSE, MOVE_SCALD, MOVE_FLAMETHROWER); Speed(2); }
        OPPONENT_B(SPECIES_WOBBUFFET) { Moves(MOVE_DRAGON_PULSE, MOVE_SCALD, MOVE_FLAMETHROWER); Speed(1); }
    } WHEN {
...
AI_TWO_VS_ONE_BATTLE_TEST("TAG TEST: 2VS1: AI: OPPONENT: SCORING: Slow KO scores +12 80% of the time for opponents")
{
    ...
    GIVEN {
        ...
        PARTNER(SPECIES_KINGDRA){ HP(1); Speed(3); };
        OPPONENT_A(SPECIES_WOBBUFFET) { Moves(MOVE_DRAGON_PULSE, MOVE_SCALD, MOVE_FLAMETHROWER); Speed(2); }
        OPPONENT_A(SPECIES_WOBBUFFET) { Moves(MOVE_DRAGON_PULSE, MOVE_SCALD, MOVE_FLAMETHROWER); Speed(1); }
    } WHEN {
            TURN { SCORE_EQ_VAL(opponentLeft, MOVE_DRAGON_PULSE, 112, target:playerLeft); 
                SCORE_EQ_VAL(opponentRight, MOVE_DRAGON_PULSE, 112, target:playerLeft);
                SCORE_EQ_VAL(opponentLeft, MOVE_DRAGON_PULSE, 112, target:playerRight); 
...
AI_MULTI_BATTLE_TEST("TAG TEST: MULTI: AI: OPPONENT: OFFENSIVE SETUP: AI +1 offensive setup is incentivised if both targets are incapacitated")
{
    GIVEN {
        AI_FLAGS(AI_FLAG_TAG_TRAINER);
        PLAYER(SPECIES_RATTATA) { Status1(STATUS1_SLEEP); Speed(4); }
        PARTNER(SPECIES_KANGASKHAN) { Status1(STATUS1_FREEZE); Speed(3); }
...
AI_MULTI_BATTLE_TEST("TAG TEST: MULTI: AI: OPPONENT: OFFENSIVE SETUP: AI +1 offensive setup is incentivised based on speed and hits to KO user")
{
    ...
    GIVEN {
        AI_FLAGS(AI_FLAG_TAG_TRAINER);
        PLAYER(SPECIES_RATTATA) { Moves(MOVE_SONIC_BOOM, MOVE_CELEBRATE); Speed(1); }
        PARTNER(SPECIES_KANGASKHAN) { Moves(MOVE_SONIC_BOOM, MOVE_CELEBRATE); Speed(2); }
        OPPONENT_A(SPECIES_WOBBUFFET) { HP(hp); Moves(MOVE_MEDITATE, MOVE_TACKLE); Speed(3); }
        OPPONENT_B(SPECIES_WYNAUT) { HP(hp); Moves(MOVE_MEDITATE, MOVE_TACKLE); Speed(4); }
    } WHEN {
            TURN { 
...
AI_TWO_VS_ONE_BATTLE_TEST("TAG TEST: 2VS1: AI: OPPONENT: OFFENSIVE SETUP: AI +1 offensive setup is incentivised based on speed and hits to KO user")
{
    ...
    GIVEN {
        ...
        PARTNER(SPECIES_KANGASKHAN) { Moves(MOVE_SONIC_BOOM, MOVE_CELEBRATE); Speed(2); }
        OPPONENT_A(SPECIES_WOBBUFFET) { HP(hp); Moves(MOVE_MEDITATE, MOVE_TACKLE); Speed(3); }
        OPPONENT_A(SPECIES_WYNAUT) { HP(hp); Moves(MOVE_MEDITATE, MOVE_TACKLE); Speed(4); }
    } WHEN {
            TURN { 
                    SCORE_EQ_VAL(opponentLeft, MOVE_MEDITATE, (hp == 100) ? 107 : 106, target:playerLeft);
                    SCORE_EQ_VAL(opponentLeft, MOVE_MEDITATE, (hp == 100) ? 107 : 106, target:playerRight); 
...
AI_MULTI_BATTLE_TEST("TAG TEST: MULTI: AI: OPPONENT: OFFENSIVE SETUP: AI always chooses +2 offensive setup over slow kill if both targets are incapacitated")
{
    GIVEN {
        AI_FLAGS(AI_FLAG_TAG_TRAINER);
        PLAYER(SPECIES_RATTATA) { HP(1); Status1(STATUS1_SLEEP); Speed(4); }
        PARTNER(SPECIES_GASTLY) { HP(1); Status1(STATUS1_FREEZE); Speed(3); }
...
AI_MULTI_BATTLE_TEST("TAG TEST: MULTI: AI: OPPONENT: OFFENSIVE SETUP: AI sometimes chooses +2 offensive setup over slow kill if one target is incapacitated")
{
    ...
    GIVEN {
        AI_FLAGS(AI_FLAG_TAG_TRAINER);
        PLAYER(SPECIES_RATTATA) { HP(1); Status1(STATUS1_SLEEP); Speed(4); }
        PARTNER(SPECIES_GASTLY) { HP(1); Speed(3); }
        OPPONENT_A(SPECIES_WOBBUFFET) { Moves(MOVE_SWORDS_DANCE, MOVE_TACKLE); Speed(2); }
        OPPONENT_B(SPECIES_WYNAUT) { Moves(MOVE_NASTY_PLOT, MOVE_SHADOW_BALL); Speed(1); }
    } WHEN {
            TURN { EXPECT_MOVE(opponentLeft, MOVE_SWORDS_DANCE); EXPECT_MOVE(opponentRight, MOVE_NASTY_PLOT); }
...
AI_MULTI_BATTLE_TEST("TAG TEST: MULTI: AI: OPPONENT: OFFENSIVE SETUP: Opponent +2 offensive setup is incentivised when the user cannot be fast 3HKO'd")
{
    GIVEN {
        AI_FLAGS(AI_FLAG_TAG_TRAINER);
        PLAYER(SPECIES_RATTATA) { Speed(2); }
        PARTNER(SPECIES_GASTLY) { Speed(1); }
...
AI_MULTI_BATTLE_TEST("TAG TEST: MULTI: AI: OPPONENT: OFFENSIVE SETUP: Opponent +2 offensive setup is slightly incentivised when the user can be fast 3HKO'd")
{
    GIVEN {
        AI_FLAGS(AI_FLAG_TAG_TRAINER);
        PLAYER(SPECIES_RATTATA) { Speed(4); Moves(MOVE_SONIC_BOOM, MOVE_CELEBRATE); }
        PARTNER(SPECIES_GASTLY) { Speed(3); Moves(MOVE_SONIC_BOOM, MOVE_CELEBRATE); }
        OPPONENT_A(SPECIES_KADABRA) { HP(60); Moves(MOVE_SWORDS_DANCE, MOVE_TACKLE); Speed(1); }
        OPPONENT_B(SPECIES_WYNAUT) { HP(60); Moves(MOVE_NASTY_PLOT, MOVE_SHADOW_BALL); Speed(2); }
    } WHEN {
...
AI_TWO_VS_ONE_BATTLE_TEST("TAG TEST: 2VS1: AI: OPPONENT: OFFENSIVE SETUP: Opponent +2 offensive setup is slightly incentivised when the user can be fast 3HKO'd")
{
    GIVEN {
        ...
        PARTNER(SPECIES_GASTLY) { Speed(3); Moves(MOVE_SONIC_BOOM, MOVE_CELEBRATE); }
        OPPONENT_A(SPECIES_KADABRA) { HP(60); Moves(MOVE_SWORDS_DANCE, MOVE_TACKLE); Speed(1); }
        OPPONENT_A(SPECIES_WYNAUT) { HP(60); Moves(MOVE_NASTY_PLOT, MOVE_SHADOW_BALL); Speed(2); }
    } WHEN {
            TURN {  SCORE_EQ_VAL(opponentLeft, MOVE_SWORDS_DANCE, 107, target:playerLeft);
                    SCORE_EQ_VAL(opponentLeft, MOVE_SWORDS_DANCE, 107, target:playerRight);
                    SCORE_EQ_VAL(opponentLeft, MOVE_TACKLE, 106, target:playerLeft);
...
AI_MULTI_BATTLE_TEST("TAG TEST: MULTI: AI: OPPONENT: DEFENSIVE SETUP: Opponent defensive setup over is incentivised based on the target's moveset")
{
    ...
    GIVEN {
        AI_FLAGS(AI_FLAG_TAG_TRAINER);
        PLAYER(SPECIES_RATTATA) { Moves(movePlayer1, movePlayer2, MOVE_CELEBRATE); }
        PARTNER(SPECIES_KANGASKHAN) { Moves(movePartner1, movePartner2, MOVE_CELEBRATE); }
        OPPONENT_A(SPECIES_WOBBUFFET) { Moves(moveOpponentA1, moveOpponentA2); }
        OPPONENT_B(SPECIES_WOBBUFFET) { Moves(moveOpponentB1, moveOpponentB2); }
    } WHEN {
            TURN {  switch(caseNo) 
                {
                    case 0:
...
AI_TWO_VS_ONE_BATTLE_TEST("TAG TEST: 2VS1: AI: OPPONENT: DEFENSIVE SETUP: Opponent defensive setup over is incentivised based on the target's moveset")
{
    ...
    GIVEN {
        ...
        PARTNER(SPECIES_KANGASKHAN) { Moves(movePartner1, movePartner2, MOVE_CELEBRATE); }
        OPPONENT_A(SPECIES_WOBBUFFET) { Moves(moveOpponentA1, moveOpponentA2); }
        OPPONENT_A(SPECIES_WOBBUFFET) { Moves(moveOpponentB1, moveOpponentB2); }
    } WHEN {
            TURN {  switch(caseNo) 
                {
                    case 0:
...
AI_MULTI_BATTLE_TEST("TAG TEST: MULTI: AI: OPPONENT: EVASION SETUP: Opponent evasion setup is less incentivised when the user can be 3HKO'd")
{
    ...
    GIVEN {
        AI_FLAGS(AI_FLAG_TAG_TRAINER);
        PLAYER(SPECIES_RATTATA) { Speed(4); Moves(MOVE_DRAGON_RAGE, MOVE_CELEBRATE); Status1(status); }
        PARTNER(SPECIES_RATTATA) { Speed(3); Moves(MOVE_DRAGON_RAGE, MOVE_CELEBRATE); Status1(status); }
        OPPONENT_A(SPECIES_WOBBUFFET) { HP(opponentHp); Moves(MOVE_DOUBLE_TEAM, MOVE_TACKLE); Speed(opponentSpeed); }
        OPPONENT_B(SPECIES_WYNAUT) { HP(opponentHp); Moves(MOVE_DOUBLE_TEAM, MOVE_TACKLE); Speed(opponentSpeed); }
    } WHEN {
            TURN {  switch(i) 
                {
                    case 0:
...
AI_TWO_VS_ONE_BATTLE_TEST("TAG TEST: 2VS1: AI: OPPONENT: EVASION SETUP: Opponent evasion setup is less incentivised when the user can be 3HKO'd")
{
    ...
    GIVEN {
        ...
        PARTNER(SPECIES_RATTATA) { Speed(3); Moves(MOVE_DRAGON_RAGE, MOVE_CELEBRATE); Status1(status); }
        OPPONENT_A(SPECIES_WOBBUFFET) { HP(opponentHp); Moves(MOVE_DOUBLE_TEAM, MOVE_TACKLE); Speed(opponentSpeed); }
        OPPONENT_A(SPECIES_WYNAUT) { HP(opponentHp); Moves(MOVE_DOUBLE_TEAM, MOVE_TACKLE); Speed(opponentSpeed); }
    } WHEN {
            TURN {  switch(i) 
                {
                    case 0:
...
AI_MULTI_BATTLE_TEST("TAG TEST: MULTI: AI: OPPONENT: SWITCHING MOVES: AI will not fast Volt Switch into a mon that is slow OHKO'd")
{
    GIVEN {
        ...
        AI_FLAGS(AI_FLAG_TAG_TRAINER);
        PLAYER(SPECIES_SANDSLASH) { HP(30); Moves(MOVE_SONIC_BOOM, MOVE_CELEBRATE); Speed(3); }
        PARTNER(SPECIES_METAGROSS) { Moves(MOVE_SONIC_BOOM, MOVE_CELEBRATE); Speed(4); }
        OPPONENT_A(SPECIES_ELECTRODE) { HP(20); Moves(MOVE_VOLT_SWITCH, MOVE_THUNDERSHOCK); Speed(6); }
        ...
        TURN { EXPECT_MOVE(opponentLeft, MOVE_VOLT_SWITCH, target:playerRight); EXPECT_SEND_OUT(opponentLeft, 2); }
    }
}
...
AI_TWO_VS_ONE_BATTLE_TEST("TAG TEST: 2VS1: AI: OPPONENT: SWITCHING MOVES: AI will not fast Volt Switch into a mon that is slow OHKO'd")
{
    GIVEN {
        ...
        PARTNER(SPECIES_METAGROSS) { Moves(MOVE_SONIC_BOOM, MOVE_CELEBRATE); Speed(4); }
        OPPONENT_A(SPECIES_ELECTRODE) { HP(20); Moves(MOVE_VOLT_SWITCH, MOVE_THUNDERSHOCK); Speed(6); }
        OPPONENT_A(SPECIES_RATTATA) { Moves(MOVE_TACKLE); Speed(5); }
        OPPONENT_A(SPECIES_WOBBUFFET) { HP(20); Moves(MOVE_SONIC_BOOM); Speed(8); }
        OPPONENT_A(SPECIES_WYNAUT) { HP(40); Moves(MOVE_SONIC_BOOM); Speed(7); }
    } WHEN {
        TURN { EXPECT_MOVE(opponentLeft, MOVE_VOLT_SWITCH, target:playerRight); EXPECT_SEND_OUT(opponentLeft, 3); }
    }
}
...
AI_MULTI_BATTLE_TEST("TAG TEST: MULTI: AI: OPPONENT: SWITCHING MOVES: AI will slow Volt Switch into a mon that is slow OHKO'd")
{
    GIVEN {
        ...
        PLAYER(SPECIES_SANDSLASH) { HP(30); Moves(MOVE_SONIC_BOOM, MOVE_CELEBRATE); Speed(3); }
        PARTNER(SPECIES_METAGROSS) { Moves(MOVE_SONIC_BOOM, MOVE_CELEBRATE); Speed(4); }
        OPPONENT_A(SPECIES_ELECTRODE) { HP(20); Moves(MOVE_VOLT_SWITCH, MOVE_THUNDERSHOCK); Speed(2); }
        ...
        TURN { EXPECT_MOVE(opponentLeft, MOVE_VOLT_SWITCH, target:playerRight); EXPECT_SEND_OUT(opponentLeft, 1); }
    }
}
...
AI_MULTI_BATTLE_TEST("TAG TEST: MULTI: AI: OPPONENT: SWITCHING MOVES: AI will not fast Volt Switch into a mon that is outsped and 2HKO'd")
{
    GIVEN {
        ...
        PARTNER(SPECIES_METAGROSS) { Moves(MOVE_SONIC_BOOM, MOVE_CELEBRATE); Speed(4); }
        OPPONENT_A(SPECIES_ELECTRODE) { HP(20); Moves(MOVE_VOLT_SWITCH, MOVE_THUNDERSHOCK); Speed(6); }
        OPPONENT_A(SPECIES_WOBBUFFET) { HP(40); Moves(MOVE_SONIC_BOOM); Speed(2); }
        OPPONENT_A(SPECIES_WYNAUT) { HP(40); Moves(MOVE_SONIC_BOOM); Speed(7); }
        OPPONENT_B(SPECIES_RATTATA) { Moves(MOVE_TACKLE); Speed(5); }
    } WHEN {
        TURN { EXPECT_MOVE(opponentLeft, MOVE_VOLT_SWITCH, target:playerRight); EXPECT_SEND_OUT(opponentLeft, 2); }
...
AI_TWO_VS_ONE_BATTLE_TEST("TAG TEST: 2VS1: AI: OPPONENT: SWITCHING MOVES: AI will not fast Volt Switch into a mon that is outsped and 2HKO'd")
{
    GIVEN {
        ...
        OPPONENT_A(SPECIES_ELECTRODE) { HP(20); Moves(MOVE_VOLT_SWITCH, MOVE_THUNDERSHOCK); Speed(6); }
        OPPONENT_A(SPECIES_RATTATA) { Moves(MOVE_TACKLE); Speed(5); }
        OPPONENT_A(SPECIES_WOBBUFFET) { HP(40); Moves(MOVE_SONIC_BOOM); Speed(1); }
        OPPONENT_A(SPECIES_WYNAUT) { HP(40); Moves(MOVE_SONIC_BOOM); Speed(7); }
    } WHEN {
        TURN { EXPECT_MOVE(opponentLeft, MOVE_VOLT_SWITCH, target:playerRight); EXPECT_SEND_OUT(opponentLeft, 3); }
    }
...
AI_MULTI_BATTLE_TEST("TAG TEST: MULTI: AI: OPPONENT: SWITCHING MOVES: AI will slow Volt Switch into a mon that is outsped and 2HKO'd")
{
    GIVEN {
        ...
        AI_FLAGS(AI_FLAG_TAG_TRAINER);
        PLAYER(SPECIES_SANDSLASH) { HP(30); Moves(MOVE_SONIC_BOOM, MOVE_CELEBRATE); Speed(3); }
        PARTNER(SPECIES_METAGROSS) { Moves(MOVE_SONIC_BOOM, MOVE_CELEBRATE); Speed(5); }
        OPPONENT_A(SPECIES_ELECTRODE) { HP(20); Moves(MOVE_VOLT_SWITCH, MOVE_THUNDERSHOCK); Speed(3); }
        OPPONENT_A(SPECIES_WOBBUFFET) { HP(40); Moves(MOVE_SONIC_BOOM); Speed(1); }
...
AI_TWO_VS_ONE_BATTLE_TEST("TAG TEST: 2VS1: AI: OPPONENT: SWITCHING MOVES: AI will slow Volt Switch into a mon that is outsped and 2HKO'd")
{
    GIVEN {
        ...
        PARTNER(SPECIES_METAGROSS) { Moves(MOVE_SONIC_BOOM, MOVE_CELEBRATE); Speed(5); }
        OPPONENT_A(SPECIES_ELECTRODE) { HP(20); Moves(MOVE_VOLT_SWITCH, MOVE_THUNDERSHOCK); Speed(3); }
        OPPONENT_A(SPECIES_RATTATA) { Moves(MOVE_TACKLE); Speed(6); }
        OPPONENT_A(SPECIES_WOBBUFFET) { HP(40); Moves(MOVE_SONIC_BOOM); Speed(1); }
        OPPONENT_A(SPECIES_WYNAUT) { HP(40); Moves(MOVE_SONIC_BOOM); Speed(2); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_CELEBRATE); EXPECT_SEND_OUT(opponentLeft, 2); }
    }
}
...
AI_MULTI_BATTLE_TEST("TAG TEST: MULTI: AI: OPPONENT: SCORING: Opponents choose priority last chance move over highest damaging move")
{
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { HP(80); Speed(3); }
        PARTNER(SPECIES_WOBBUFFET) { HP(80); Moves(MOVE_DRAGON_RAGE, MOVE_CELEBRATE); Speed(3); }
        OPPONENT_A(SPECIES_KINGDRA) { HP(40); Moves(MOVE_DRAGON_RAGE, MOVE_MACH_PUNCH); Speed(2); };
        OPPONENT_B(SPECIES_KINGDRA) { HP(40); Moves(MOVE_DRAGON_RAGE, MOVE_MACH_PUNCH); Speed(2); };
    } WHEN {
...
/*AI_MULTI_BATTLE_TEST("TAG TEST: MULTI: AI: OPPONENT: SCORING: Opponents choose priority last chance move over slow KO 90% of the time")
{
    ...
    GIVEN {
        ...
        TIE_BREAK_SCORE(RNG_AI_SCORE_TIE_DOUBLES_MOVE, SCORE_TIE_CHOSEN, (i / (2 * i)));
        PLAYER(SPECIES_VESPIQUEN) { HP(150); Speed(3); }
        PARTNER(SPECIES_VESPIQUEN) { HP(100); Moves(MOVE_NIGHT_SHADE, MOVE_CELEBRATE); Speed(3); }
        OPPONENT_A(SPECIES_WOBBUFFET) { HP(30); Moves(MOVE_NIGHT_SHADE, MOVE_MACH_PUNCH); Speed(1); };
        OPPONENT_B(SPECIES_WOBBUFFET) { HP(30); Moves(MOVE_NIGHT_SHADE, MOVE_MACH_PUNCH); Speed(2); };
    } WHEN {
            TURN { MOVE(playerRight, MOVE_CELEBRATE); EXPECT_MOVE(opponentRight, MOVE_MACH_PUNCH, target:playerRight); }
        } 
}
...
AI_TWO_VS_ONE_BATTLE_TEST("TAG TEST: 2VS1: AI: OPPONENT: SCORING: Opponents choose priority last chance move over slow KO 90% of the time")
{
    ...
    GIVEN {
        ...
        BATTLER_AI_FLAGS(opponentRight, AI_FLAG_TAG_TRAINER);
        TIE_BREAK_TARGET(TARGET_TIE_HI, 0);
        TIE_BREAK_SCORE(RNG_AI_SCORE_TIE_DOUBLES_MOVE, SCORE_TIE_RANDOM, (i / (2 * i)));
        PLAYER(SPECIES_VESPIQUEN) { HP(150); Speed(3); }
        PARTNER(SPECIES_VESPIQUEN) { HP(100); Moves(MOVE_NIGHT_SHADE, MOVE_CELEBRATE); Speed(3); }
        OPPONENT_A(SPECIES_WOBBUFFET) { HP(30); Moves(MOVE_NIGHT_SHADE, MOVE_MACH_PUNCH); Speed(1); };
        OPPONENT_A(SPECIES_WOBBUFFET) { HP(30); Moves(MOVE_NIGHT_SHADE, MOVE_MACH_PUNCH); Speed(2); };
    } WHEN {
            TURN { MOVE(playerRight, MOVE_CELEBRATE); EXPECT_MOVE(opponentRight, MOVE_MACH_PUNCH, target:playerRight); }
        } 
}*/
...
AI_MULTI_BATTLE_TEST("TAG TEST: MULTI: AI: OPPONENT: SCORING: AI opponents can click Coaching/Enlightening")
{
    ...
    GIVEN {
        ...
        BATTLER_AI_FLAGS(playerRight, AI_FLAG_PARTNER_TRAINER);
        BATTLER_AI_FLAGS(opponentRight, AI_FLAG_TAG_TRAINER);
        PLAYER(SPECIES_WOBBUFFET) { Moves(playerMove, MOVE_CELEBRATE); }
        PARTNER(SPECIES_WOBBUFFET);
        OPPONENT_A(SPECIES_KINGDRA) { Moves(moveOpponentA1, moveOpponentA2); }
        OPPONENT_B(SPECIES_KINGDRA) { Moves(moveOpponentB1, moveOpponentB2); }
    } WHEN {
            TURN {  switch(caseNo)
                    {
                        case 3:
...
AI_TWO_VS_ONE_BATTLE_TEST("TAG TEST: 2VS1: AI: OPPONENT: SCORING: AI opponents can click Coaching/Enlightening")
{
    ...
    GIVEN {
        ...
        PARTNER(SPECIES_WOBBUFFET);
        OPPONENT_A(SPECIES_KINGDRA) { Moves(moveOpponentA1, moveOpponentA2); }
        OPPONENT_A(SPECIES_KINGDRA) { Moves(moveOpponentB1, moveOpponentB2); }
    } WHEN {
            TURN {  switch(caseNo)
                    {
                        case 3:
...
AI_MULTI_BATTLE_TEST("TAG TEST: MULTI: AI: OPPONENT: SCORING: AI opponents will not click Coaching/Enlightening on a Contrary ally")
{
    ...
    GIVEN {
        ...
        PARTNER(SPECIES_WOBBUFFET);
        OPPONENT_A(SPECIES_SERPERIOR) { Moves(moveOpponentA1, moveOpponentA2); Ability(abilityOpponentA); }
        OPPONENT_B(SPECIES_SERPERIOR) { Moves(moveOpponentB1, moveOpponentB2); Ability(abilityOpponentB); }
    } WHEN {
            TURN {  switch(i)
                    {
                        case 1:
...
AI_TWO_VS_ONE_BATTLE_TEST("TAG TEST: 2VS1: AI: OPPONENT: SCORING: AI opponents will not click Coaching/Enlightening on a Contrary ally")
{
    ...
    GIVEN {
        ...
        PARTNER(SPECIES_WOBBUFFET);
        OPPONENT_A(SPECIES_SERPERIOR) { Moves(moveOpponentA1, moveOpponentA2); Ability(abilityOpponentA); }
        OPPONENT_A(SPECIES_SERPERIOR) { Moves(moveOpponentB1, moveOpponentB2); Ability(abilityOpponentB); }
    } WHEN {
            TURN {  switch(i)
                    {
                        case 1:
...
AI_MULTI_BATTLE_TEST("TAG TEST: MULTI: AI: OPPONENT: SCORING: AI opponent Fake Out scoring compounds with best damage move")
{
    GIVEN {
        ...
        PLAYER(SPECIES_AGGRON);
        PARTNER(SPECIES_LAIRON);
        OPPONENT_A(SPECIES_WOBBUFFET) { Moves(MOVE_CONSTRICT, MOVE_FAKE_OUT); }
        OPPONENT_B(SPECIES_WOBBUFFET) { Moves(MOVE_CONSTRICT, MOVE_FAKE_OUT); }
    } WHEN {
        TURN { EXPECT_MOVE(opponentLeft, MOVE_FAKE_OUT, target:playerRight); EXPECT_MOVE(opponentRight, MOVE_FAKE_OUT, target:playerRight);
...
AI_MULTI_BATTLE_TEST("TAG TEST: MULTI: AI: OPPONENT: SCORING: AI opponent Fake Out scoring compounds with last chance")
{
    GIVEN {
        ...
        BATTLER_AI_FLAGS(opponentRight, AI_FLAG_TAG_TRAINER);
        TIE_BREAK_TARGET(TARGET_TIE_HI, 0);
        PLAYER(SPECIES_AGGRON) { Speed(3); }
        PARTNER(SPECIES_LAIRON) { Speed(3); Moves(MOVE_DRAGON_RAGE, MOVE_CELEBRATE); HP(200); }
        OPPONENT_A(SPECIES_WOBBUFFET) { Speed(2); Moves(MOVE_SEISMIC_TOSS, MOVE_FAKE_OUT); HP(1); }
