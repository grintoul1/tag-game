...
SINGLE_BATTLE_TEST("Hail Warning sets up hail for 5 turns (Gen6-8)")
{
    GIVEN {
        WITH_CONFIG(CONFIG_SNOW_WARNING, GEN_8);
        WITH_CONFIG(CONFIG_ABILITY_WEATHER, GEN_6);
        PLAYER(SPECIES_ABOMASNOW) { Moves(MOVE_CELEBRATE); Ability(ABILITY_HAIL_WARNING); }
        ...
        ABILITY_POPUP(player, ABILITY_HAIL_WARNING);
...
SINGLE_BATTLE_TEST("Hail Warning sets up hail for 8 turns with Icy Rock (Gen6-8)")
{
    GIVEN {
        WITH_CONFIG(CONFIG_SNOW_WARNING, GEN_8);
        WITH_CONFIG(CONFIG_ABILITY_WEATHER, GEN_6);
        PLAYER(SPECIES_ABOMASNOW) { Moves(MOVE_CELEBRATE); Ability(ABILITY_HAIL_WARNING); Item(ITEM_ICY_ROCK); }
        OPPONENT(SPECIES_WOBBUFFET) { Moves(MOVE_CELEBRATE); }
    } WHEN {
        TURN { MOVE(player, MOVE_CELEBRATE); MOVE(opponent, MOVE_CELEBRATE); }
        TURN { MOVE(player, MOVE_CELEBRATE); MOVE(opponent, MOVE_CELEBRATE); }
...
SINGLE_BATTLE_TEST("Hail Warning sets up permanent hail (Gen4-5)")
{
    GIVEN {
        WITH_CONFIG(CONFIG_SNOW_WARNING, GEN_8);
