...
AI_SINGLE_BATTLE_TEST("AI will not use Throat Chop if opposing mon has a better move")
{
    GIVEN {
        ...
        ASSUME(GetMovePower(MOVE_DISARMING_VOICE) == 60);
...
AI_SINGLE_BATTLE_TEST("AI_IsMoveEffectInPlus - AI should not see secondary effect of Sheer Force boosted moves as beneficial")
{
    GIVEN {
        ...
        TURN { MOVE(player, MOVE_GYRO_BALL); SCORE_EQ_VAL(opponent, MOVE_PSYCHIC, AI_SCORE_DEFAULT + BEST_DAMAGE_MOVE); SCORE_EQ_VAL(opponent, MOVE_NIGHT_SHADE, AI_SCORE_DEFAULT + BEST_DAMAGE_MOVE); }
    }
}
