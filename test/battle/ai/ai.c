...
AI_SINGLE_BATTLE_TEST("TIE_BREAK_SCORE can control AI move selection when scores are tied (Singles)")
{
    u32 tiedMove;
    u16 expectedMove;
...
AI_DOUBLE_BATTLE_TEST("TIE_BREAK_SCORE can control AI move selection when scores are tied (Doubles)")
{
    u32 tiedMove;
    u16 expectedMove;
    PARAMETRIZE { tiedMove = 3; expectedMove = MOVE_ICE_BEAM;       }
...
AI_SINGLE_BATTLE_TEST("TIE_BREAK_SCORE correctly controls AI move selection when scores are tied for all values in enum ScoreTieResolution (Singles)")
{
    ...
    PARAMETRIZE { enumValue = SCORE_TIE_RANDOM; expectedMove = MOVE_THUNDERBOLT;    }
...
AI_DOUBLE_BATTLE_TEST("TIE_BREAK_TARGET can correctly control AI target selection when scores are tied")
{
    u32 targetValue;
...
AI_DOUBLE_BATTLE_TEST("TIE_BREAK_TARGET correctly controls AI target selection when scores are tied for all values in enum TargetTieResolution")
{
    ...
    PARAMETRIZE { enumValue = TARGET_TIE_RANDOM; targetValue = 0; expectedTarget = playerLeft;    }
