...
#SCRIPT    := $(TOOLS_DIR)/poryscript/poryscript$(EXE) #poryscript
...
#AUTO_GEN_TARGETS += $(patsubst %.pory,%.inc,$(shell find data/ -type f -name '*.pory')) #poryscript
...
%.smol:     %      ; $(SMOL) -w $< $@
%.rl:     %      ; $(GFX) $< $@
#data/%.inc: data/%.pory; $(SCRIPT) -i $< -o $@ -fc tools/poryscript/font_config.json -cc tools/poryscript/command_config.json #poryscript
